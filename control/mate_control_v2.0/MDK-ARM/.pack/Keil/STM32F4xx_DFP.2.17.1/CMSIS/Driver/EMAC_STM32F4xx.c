...
#define ARM_ETH_MAC_DRV_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,10) /* driver version */
...
static uint32_t GetRxFrameSize (void) {
  volatile uint32_t stat = rx_desc[Emac.rx_index].Stat;
...
#if (EMAC_TIME_STAMP)
  volatile RX_Desc *rxd = &rx_desc[Emac.rx_index];
...
#if (EMAC_TIME_STAMP)
  volatile TX_Desc *txd = &tx_desc[Emac.tx_ts_index];
